...
class DDPG(object):
    ...
    def critic_learn(self):
        ...
        na1=targettable[(np.rint(bs_[:,3])-index_0).astype(int),(np.rint(bs_[:,4])-index_0).astype(int),0]
        na2=targettable[(np.rint(bs_[:,3])-index_0).astype(int),(np.rint(bs_[:,4])-index_0).astype(int),1]
        next_action=[na1,na2,env.nbikes-na1-na2]
        
        next_action=np.asarray(next_action).T
        self.sess.run(self.ctrain, {self.S: bs, self.a: ba, self.R: br, self.S_: bs_,self.a_:next_action})
...
class DDPG_action(object):       
    ...
    def update(self,ddpg,lr):
        for s1 in range(16):
            for s2 in range(16):
                a=table[s1,s2]
                gradient=ddpg.get_gradient([[s1,s1,s1,s1+20,s2+20,env.nbikes-s1-s2-40,s1]],[a])
...
for ep in range(episode):
    ...
    while not done:
        ...
        if np.random.random() >= exploration:
            a1,a2=DDPGaction.DDPG_policy(s)        #here need to change to tabuler
            a1=np.rint(a1)
            a2=np.rint(a2)
            action=[a1, a2, env.nbikes - a1 - a2]
                ...
                for j in range(20, 36):
                    for k in range(20, 36):
                        if (i+j+k==90):
                            ...
                            a3=90-a1-a2
                            norm_1=norm_1+np.sqrt(abs(a1-i)**2+abs(a2-j)**2+abs(a3-k)**2)
            norm.append(norm_1)
...
np.save("Bike_{}_DDPGProjection_Tabular_Reward".format(arg_seed),Rs)
np.save("Bike_{}_DDPGProjection_Tabular_before_Action".format(arg_seed),store_before_action)
np.save("Bike_{}_DDPGProjection_Tabular_After_Action".format(arg_seed),store_after_action)
np.save("Bike_{}_DDPGProjection_Tabular_eval_reward".format(arg_seed),eva_reward)
np.save("Bike_{}_DDPGProjection_Tabular_eval_before_Action".format(arg_seed),store_testing_before_action)
np.save("Bike_{}_DDPGProjection_Tabular_eval_after_Action".format(arg_seed),store_testing_after_action)
np.save("Bike_{}_DDPGProjection_Tabular_table".format(arg_seed),table)
np.save("Bike_{}_DDPGProjection_Tabular_norm".format(arg_seed),norm)
