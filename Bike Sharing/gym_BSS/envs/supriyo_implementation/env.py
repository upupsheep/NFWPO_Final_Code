...
class BSSEnv(gym.Env):
    ...
    def _get_observation(self):
        if self.__t == 0:
            demand_2d = np.zeros(shape=[self.nzones, self.nzones])  #initial demand_2d = 0
        else:
    ...
    def __reset_allocation(self):  #reset allocation to initial [0][0~95]有值 其他0
    ...
    def __reset_flow(self, scenario):
        ...
        for i in range(self.ntimesteps):
            for j in range(self.nzones):
                for k in range(self.nzones):
                    demand_flow[i][j][k] = float(np.random.randint(5, 25)) if j != k else 0.0
    ...
    def __calculate_lost_demand_new_allocation(self):
       ...
       # print("calculatetime",iteration)
        ...
        before_reallocation = sum(self.__ds[iteration]) #before_reallocation=760
            ...
            if(self.__tfl1[iteration][s] > 0 and (self.__ds[iteration][s])<sum(self.__fl[iteration][s])):
                ...
                for find_last in range(self.nzones-1,-1,-1):
                    if(self.__xfl[iteration][s][find_last]!=0):
                        self.__xfl[iteration][s][find_last]=self.__ds[iteration][s]-sum(self.__xfl[iteration][s][0:find_last])
                        break
