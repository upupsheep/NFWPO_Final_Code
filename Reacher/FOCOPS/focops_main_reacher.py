...
class FOCOPS:
    ...
    def update_params(self, rollout, dtype, device):
        ...
        np.save('focops_results_reacher/avg_cost_state_{}'.format(random_seed), avg_costs)
        # Update nu
...
def train(args):
    ...
    for iter in range(args.max_iter_num):
        ...
        network_states_.append(rollout['network_states'])
        np.save('focops_results_reacher/network_states_state_{}'.format(random_seed), network_states_)
        network_actions_.append(rollout['network_actions'])
        np.save('focops_results_reacher/network_actions_state_{}'.format(random_seed), network_actions_)
        # Update FOCOPS parameters
        agent.update_params(rollout, dtype, device)
        if( (iter+1) % 5 ==0):	
            eval_rewards.append(data_generator.evaluation(env, agent.policy, agent.value_net, agent.cvalue_net,
                                          running_stat, agent.score_queue, agent.cscore_queue,
                                          args.gamma, args.c_gamma, args.gae_lam, args.c_gae_lam,
                                          dtype, device, args.constraint))
        np.save('focops_results_reacher/eval_reward_state_{}'.format(random_seed), eval_rewards)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env-id', default='Reacher-v2',
                        help='Name of Environment (default: Humanoid-v3')
    parser.add_argument('--constraint', default='state',
    ...
    parser.add_argument('--max-eps-len', type=int, default=50,
    ...
    parser.add_argument('--max-iter-num', type=int, default=6000,
