...
from TRPO_Projection_Reahcer_gym import ClippedGaussianPolicy
...
def main():
    ...
    parser.add_argument('--steps', type=int, default=3*10 ** 5,
    ...
    def OptLayer_function(action):
        with gp.Env(empty=True) as env:
            ...
            with gp.Model(env=env) as reacher_m:
                ...
                #reacher_m.addConstr(a1+a2<=0.1)   #x+y+z==90
...
to numpy.ndarray of numpy.float32.""")  # NOQA
    ...
    if args.demo:
        ...
        np.save("Reacher_{}_PPO_Evaluation_Reward_State_relate".format(args.seed),eva_reward)
        np.save("Reacher_{}_PPO_Action_before_State_relate".format(args.seed),action_before)
        np.save("Reacher_{}_PPO_Action_After_State_relate".format(args.seed),action_after)
        np.save("Reacher_{}_PPO_Training_Reward_State_relate".format(args.seed),reward)
