...
def sac(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        ...
        polyak=0.995, lr=0.0005, alpha=0.2, batch_size=100, start_steps=1000, 
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            if SAVE_FILE:
                np.save(
                    filepath+'/reacher_seed{}_ewma_reward'.format(seed), np.array(store_ewma))
                np.save(
                    filepath+'/reacher_seed{}_after_action'.format(seed), np.array(store_after_action))
                np.save(
                    filepath+'/reacher_seed{}_before_action'.format(seed), np.array(store_before_action))
                np.save(
                    filepath+'/reacher_seed{}_state'.format(seed), np.array(store_state))
        ...
        if (t+1) % eval_freq == 0:
            ...
            if SAVE_FILE:
                np.save(filepath+'/reacher_seed{}_eval_reward'.format(seed), np.array(store_eval_reward))
