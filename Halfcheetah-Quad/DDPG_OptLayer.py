...
def eval_policy(policy, env_name, seed, eval_episodes=10):
    ...
    if LOAD_MODEL:
        eval_action = list(
            np.load(filepath+'/cheetah_con1_seed{}_eval_action.npy'.format(random_seed)))
    ...
    print("---------------------------------------")
    if SAVE_FILE:
        np.save(filepath+'/cheetah_con1_seed{}_eval_action'.format(random_seed),
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as half_m:
            ...
            for i in range(batch_size):
                ...
                obj = (a1-joint1_a1)**2 + (a2-joint1_a2)**2 + (a3-joint2_a1)**2 + \
                    (a4-joint2_a2)**2 + (a5-joint3_a1)**2 + (a6-joint3_a2)**2
                ...
                if random_sample:
                    rtn_action = np.append(
                        rtn_action, [a1.X, a2.X, a3.X, a4.X, a5.X, a6.X])
                else:
...
if __name__ == "__main__":
    ...
    if LOAD_MODEL:
        ...
        store_reward = list(
            np.load(filepath+'/cheetah_con1_seed{}_episode_reward.npy'.format(random_seed)))
        store_ewma = list(
            np.load(filepath+'/cheetah_con1_seed{}_ewma_reward.npy'.format(random_seed)))
        store_action = list(
            np.load(filepath+'/cheetah_con1_seed{}_action.npy'.format(random_seed)))
        before_opt = list(np.load(
            filepath+'/cheetah_con1_seed{}_before_opt.npy'.format(random_seed), allow_pickle=True))
        after_opt = list(np.load(
            filepath+'/cheetah_con1_seed{}_after_opt.npy'.format(random_seed), allow_pickle=True))
        evaluations = list(
            np.load(filepath+'/cheetah_con1_seed{}_eval_reward.npy'.format(random_seed)))
    ...
    try:
        ...
        for t in range(int(MAX_EPISODES)):
            ...
            if done:
                ...
                if SAVE_FILE:
                    np.save(
                        filepath+'/cheetah_con1_seed{}_ewma_reward'.format(random_seed), np.array(store_ewma))
                    np.save(
                        filepath+'/cheetah_con1_seed{}_action'.format(random_seed), np.array(store_action))
                    np.save(
                        filepath+'/cheetah_con1_seed{}_before_opt'.format(random_seed), before_opt)
                    np.save(
                        filepath+'/cheetah_con1_seed{}_after_gaussian'.format(random_seed), after_gaussian)
                    np.save(
                        filepath+'/cheetah_con1_seed{}_eval_before_opt'.format(random_seed), eval_before_opt)
                    np.save(
                        filepath+'/cheetah_con1_seed{}_eval_after_opt'.format(random_seed), eval_after_opt)
            ...
            if (t + 1) % eval_freq == 0:
                ...
                if SAVE_FILE:
                    np.save(
                        filepath+'/cheetah_con1_seed{}_eval_reward'.format(random_seed), np.array(evaluations))
        ...
        if SAVE_MODEL:
            policy.save(f"./models/{file_name}")
        save_object(
            replay_buffer, './replay_buffer_cheetah_con1_gaussian_seed{}.pkl'.format(random_seed))
        traceback.print_exc()
