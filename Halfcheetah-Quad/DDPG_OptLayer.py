...
SAVE_FILE = True
filepath = "./results_cheetah_con1_gaussian"
if not os.path.exists(filepath):
    os.makedirs(filepath)

SAVE_MODEL = True
LOAD_MODEL = False
file_name = "cheetah_con1_gaussian_seed{}".format(random_seed)
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
    ...
    if LOAD_MODEL:
        eval_action = list(np.load(filepath+'/cheetah_con1_seed{}_eval_action.npy'.format(random_seed)))
    ...
    print("---------------------------------------")
    if SAVE_FILE:
        np.save(filepath+'/cheetah_con1_seed{}_eval_action'.format(random_seed), np.array(eval_action))
        # np.save(filepath+'/cheetah_con1_seed{}_eval_state'.format(random_seed), np.array(eval_state))
    return avg_reward
    ...
    with gp.Env(empty=True) as env:
        ...
        with gp.Model(env=env) as half_m:
            ...
            for i in range(batch_size):
                ...
                # print('a1.x: ', a1.x)
                if random_sample:
                    rtn_action = np.append(rtn_action, [a1.X, a2.X, a3.X, a4.X, a5.X, a6.X])
                else:
                    batch_opt_a = torch.tensor([a1.X, a2.X, a3.X, a4.X, a5.X, a6.X])
...
class OptLayer(torch.nn.Module):
    def __init__(self, D_in, D_out, state_dim):
        ...
        cons = [cp.sum_squares(x[:3]) <= 1, 
                cp.sum_squares(x[3:]) <= 1]

        prob = cp.Problem(obj, cons)
        self.layer = CvxpyLayer(prob, parameters=[q], variables=[x])
...
if __name__ == "__main__":
    ...
    if LOAD_MODEL:
        ...
        with open('replay_buffer_cheetah_con1_gaussian_seed{}.pkl'.format(random_seed), 'rb') as input:
            replay_buffer = pickle.load(input)
        print("==========end load")
        print("r size: ", replay_buffer.size)

        store_reward = list(np.load(filepath+'/cheetah_con1_seed{}_episode_reward.npy'.format(random_seed)))
        store_ewma = list(np.load(filepath+'/cheetah_con1_seed{}_ewma_reward.npy'.format(random_seed)))
        store_action = list(np.load(filepath+'/cheetah_con1_seed{}_action.npy'.format(random_seed)))
        before_opt = list(np.load(filepath+'/cheetah_con1_seed{}_before_opt.npy'.format(random_seed), allow_pickle = True))
        after_opt = list(np.load(filepath+'/cheetah_con1_seed{}_after_opt.npy'.format(random_seed), allow_pickle = True))
        evaluations = list(np.load(filepath+'/cheetah_con1_seed{}_eval_reward.npy'.format(random_seed)))
    ...
    try:
        ...
        for t in range(int(MAX_EPISODES)):
            with torch.no_grad():
                ...
                sele_time = time.time()
                if t < 10000:
                    #print('cheetah env sample: ', env.action_space.sample())
                    action_env = np.array([env.action_space.sample()])
            ...
            if t % 50 == 0:
                print("t: ", t)
                # save_object(replay_buffer, 'replay_buffer_cheetah_con1_gaussian.pkl')
            ...
            if done: 
                ...
                if SAVE_FILE:
                    np.save(filepath+'/cheetah_con1_seed{}_ewma_reward'.format(random_seed), np.array(store_ewma))
                    np.save(filepath+'/cheetah_con1_seed{}_action'.format(random_seed), np.array(store_action))
                    np.save(filepath+'/cheetah_con1_seed{}_before_opt'.format(random_seed), before_opt)
                    np.save(filepath+'/cheetah_con1_seed{}_after_gaussian'.format(random_seed), after_gaussian)
                    np.save(filepath+'/cheetah_con1_seed{}_eval_before_opt'.format(random_seed), eval_before_opt)
                    np.save(filepath+'/cheetah_con1_seed{}_eval_after_opt'.format(random_seed), eval_after_opt)
            ...
            if (t + 1) % eval_freq == 0:
                ...
                if SAVE_FILE:
                    np.save(filepath+'/cheetah_con1_seed{}_eval_reward'.format(random_seed), np.array(evaluations))
                # np.save(f"./results/{file_name}", evaluations)
                # if SAVE_MODEL: policy.save(f"./models/{file_name}")

            # if SAVE_FILE:
            #     np.save(filepath+'/cheetah_con1_seed{}_episode_reward'.format(random_seed), np.array(store_reward))
            #     np.save(filepath+'/cheetah_con1_seed{}_ewma_reward'.format(random_seed), np.array(store_ewma))
            #     np.save(filepath+'/cheetah_con1_seed{}_action'.format(random_seed), np.array(store_action))
            #     np.save(filepath+'/cheetah_con1_seed{}_before_opt'.format(random_seed), before_opt)
            #     np.save(filepath+'/cheetah_con1_seed{}_after_opt'.format(random_seed), after_opt)
    
    except:
        print("SAVING...")
        if SAVE_MODEL: policy.save(f"./models/{file_name}")
        save_object(replay_buffer, './replay_buffer_cheetah_con1_gaussian_seed{}.pkl'.format(random_seed))
        traceback.print_exc()