...
for ep in range(1000000):
    ...
    while not done:
        ...
        ### the loss rescale to the same scale as r###
        loss=max(0,loss)
        loss=np.sqrt(loss)*2
        lossnp.append(loss)
...
np.save("Halfcheetah_{}_RewardShape_Reward_state_relate".format(arg_seed),reward)
np.save("Halfcheetah_{}_RewardShape_before_Action_state_relate".format(arg_seed),store_before_action)
np.save("Halfcheetah_{}_RewardShape_after_Action_state_relate".format(arg_seed),store_after_action)
np.save("Halfcheetah_{}_RewardShape_eval_reward_state_relate".format(arg_seed),eva_reward)
np.save("Halfcheetah_{}_RewardShape_before_Action_Gaussian_state_relate".format(arg_seed),store_before_action_and_gaussain)
np.save("Halfcheetah_{}_RewardShape_eval_before_Action_state_relate".format(arg_seed),store_testing_before_action)
np.save("Halfcheetah_{}_RewardShape_eval_After_Action_state_relate".format(arg_seed),store_testing_after_action)
np.save("Halfcheetah_{}_RewardShape_loss_state_relate".format(arg_seed),lossnp)
