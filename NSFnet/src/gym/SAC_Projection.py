...
os.environ['CUDA_VISIBLE_DEVICES'] = '2' #make render not lag
...
def sac(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        eval_freq=10000, epochs=100, replay_size=int(5e4), gamma=0.99, 
        polyak=0.995, lr=0.0005, alpha=0.2, batch_size=16, start_steps=10000, 
        update_after=10000, update_every=50, num_test_episodes=10, 
    ...
    proj_pi_next = core.placeholders(act_dim)[0]
    ...
    with tf.compat.v1.variable_scope('target'):
        ...
        # Project pi_next
        _, _, _, q1_targ, q2_targ  = actor_critic(x2_ph, proj_pi_next, **ac_kwargs)
    ...
    # action_grad = tf.gradients(min_q_pi, pi)
    ...
    def test_agent_stoch():
        ...
        print("------------------------------------------------")
        print("(Stochastic) Evaluation average reward :",avgreward)
        print("------------------------------------------------")
        return avgreward

    def test_agent_deter():
        test_env.seed(random_seed + 100)
        avgreward = 0
        for j in range(num_test_episodes):
            o, d, ep_ret, ep_len = test_env.reset(), False, 0, 0
        ...
        avgreward = avgreward / num_test_episodes
        print("------------------------------------------------")
        print("(Deterministic) Evaluation average reward :",avgreward)
        print("------------------------------------------------")
        return avgreward
    ...
    for t in range(total_steps):
        ...
        # check_sum = (a[0]**2 + a[1]**2 + a[2]**2 + a[3]**2 + a[4]**2 + a[5]**2)
        # assert check_sum <= 1+1e-6, 'Must le 1, sum = %d!!!' % check_sum
        ...
        if d or (ep_len == max_ep_len):
            ...
            if SAVE_FILE:
                np.save(
                    filepath+'/network_seed{}_ewma_reward'.format(seed), np.array(store_ewma))
                np.save(
                    filepath+'/network_seed{}_after_action'.format(seed), np.array(store_after_action))
                np.save(
                    filepath+'/network_seed{}_before_action'.format(seed), np.array(store_before_action))
                np.save(
                    filepath+'/network_seed{}_state'.format(seed), np.array(store_state))
                np.save(
                    filepath+'/network_seed{}_q1'.format(seed), np.array(store_q1))
                np.save(
                    filepath+'/network_seed{}_q2'.format(seed), np.array(store_q2))
                # np.save(
                #     filepath+'/network_seed{}_grad'.format(seed), np.array(store_grad))
        ...
        if t >= update_after and t % update_every == 0:
            for j in range(update_every):
                ...
                # Project pi_next
                np_pinext = sess.run(pi_next, {x2_ph: batch['obs2'], a_ph: batch['acts'],})
                # print('np_pinext: ', np_pinext)
                # proj_np_pinext = Proj_batch(np_pinext)
                proj_np_pinext = []
                for i in range(batch_size):
                    proj_np_pinext.append(Projection(np_pinext[i]))
                proj_np_pinext = np.array(proj_np_pinext)
                ...
                feed_dict = {x_ph: batch['obs1'],
                             ...
                             r_ph: batch['rews'],
                             d_ph: batch['done'],
                             proj_pi_next: proj_np_pinext,
                            }
                outs = sess.run(step_ops, feed_dict)

                # a_grad_val = np.array((sess.run(action_grad, feed_dict))[0])
            q1_norm = np.linalg.norm(outs[3])
            q2_norm = np.linalg.norm(outs[4])
            store_q1.append(q1_norm)
            store_q2.append(q2_norm)

            # grad_norm = np.linalg.norm(a_grad_val, axis=1) # l2-norm for each batch (size: a_dim)
            # grad_batch_avg = np.average(grad_norm) # batch avg grad
            # store_grad.append(grad_batch_avg)
            print('============================')
            print('t: {}, q1: {}, q2: {}'.format(t, q1_norm, q2_norm))
            # print('action_grad_val: ', a_grad_val)
            # print('action_grad: ', grad_batch_avg)
            print('============================')
              
        # End of epoch wrap-up
        if (t+1) % eval_freq == 0:
            # Test the performance of the deterministic version of the agent.
            store_eval_deter.append(test_agent_deter())
            if SAVE_FILE:
                np.save(filepath+'/network_seed{}_eval_deter_reward'.format(seed), np.array(store_eval_deter))

            store_eval_stoch.append(test_agent_stoch())
            if SAVE_FILE:
                np.save(filepath+'/network_seed{}_eval_stoch_reward'.format(seed), np.array(store_eval_stoch))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='PccNs_uti_NSFnet_multiV2-v0')
    ...
    filepath = "/708HDD/upupsheep/SAC_NFWPO/NSFnet/results_network_sac_TargetProj_b100_upd10"
