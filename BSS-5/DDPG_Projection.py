...
def evaluation(env_name, seed, ddpg, eval_episode=10):
    ...
    for eptest in range(eval_episode):
        ...
        while not done:
            ...
            action, loss = Projection_function(action)
...
class DDPG(object):
    def __init__(self, a_dim, s_dim, a_bound):
        ...
        self.atrain = tf.compat.v1.train.AdamOptimizer(
            LR_A).minimize(self.a_loss, var_list=self.ae_params)
    ...
    def _build_a(self, s, scope, trainable):
        with tf.compat.v1.variable_scope(scope):
            net = tf.compat.v1.layers.dense(
                s, 400, activation=tf.nn.leaky_relu, name='l1', trainable=trainable)
            net1 = tf.compat.v1.layers.dense(
                net, 300, activation=tf.nn.leaky_relu, name='l2', trainable=trainable)
            a = tf.compat.v1.layers.dense(
                net1, self.a_dim, activation=tf.nn.tanh, name='a', trainable=trainable)
    ...
    def _build_c(self, s, a, scope, trainable):
        with tf.compat.v1.variable_scope(scope):
            net = tf.compat.v1.layers.dense(
                s, 400, activation=tf.nn.leaky_relu, name='cl1', trainable=trainable)
            ...
            net2 = tf.nn.leaky_relu(
                tf.matmul(a, w2_a)+tf.matmul(net, w2_net)+b2)
            # Q(s,a)
            return tf.compat.v1.layers.dense(net2, 1, trainable=trainable)
#############################  Projection  ####################################


def Projection_function(action):
...
for ep in range(episode):
    ...
    while not done:
        ...
        if ddpg.pointer < 10000:
            ...
            action = [valid[chose][0], valid[chose][1],
                      valid[chose][2], valid[chose][3], valid[chose][4]]
...
np.save("Bike_{}_DDPGProjection_Reward".format(arg_seed), Rs)
np.save("Bike_{}_DDPGProjection_before_Action".format(
    arg_seed), store_before_action)
np.save("Bike_{}_DDPGProjection_after_Action".format(
    arg_seed), store_after_action)
np.save("Bike_{}_DDPGProjection_eval_reward".format(arg_seed), eva_reward)
np.save("Bike_{}_DDPGProjection_eval_before_Action".format(
    arg_seed), store_testing_before_action)
np.save("Bike_{}_DDPGProjection_eval_after_Action".format(
    arg_seed), store_testing_after_action)
np.save("Bike_{}_DDPGProjection_before_Action_Gaussian".format(
