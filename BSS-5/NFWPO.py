        ...
        for i in range(BATCH_SIZE):
            with gp.Env(empty=True) as env:
                ...
                with gp.Model(env=env) as fw_m:
                    a1 = fw_m.addVar(lb=0, ub=35, name="a1",
                                     vtype=GRB.CONTINUOUS)
                    a2 = fw_m.addVar(lb=0, ub=35, name="a2",
                                     vtype=GRB.CONTINUOUS)
                    a3 = fw_m.addVar(lb=0, ub=35, name="a3",
                                     vtype=GRB.CONTINUOUS)
                    a4 = fw_m.addVar(lb=0, ub=35, name="a4",
                                     vtype=GRB.CONTINUOUS)
                    a5 = fw_m.addVar(lb=0, ub=35, name="a5",
                                     vtype=GRB.CONTINUOUS)

                    obj = a1*grad[i][0]+a2*grad[i][1]+a3 * \
                        grad[i][2]+a4*grad[i][3]+a5*grad[i][4]
...
np.save("Bike_{}_DDPGFW_before_Action_Gaussian".format(
    arg_seed), store_before_action_and_gaussain)
np.save("Bike_{}_DDPGFW_eval_before_Action".format(
    arg_seed), store_testing_before_action)
np.save("Bike_{}_DDPGFW_eval_After_Action".format(
