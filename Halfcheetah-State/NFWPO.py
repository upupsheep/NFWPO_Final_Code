...
def evaluation(env_name, seed, ddpg, eval_episode=10):
    ...
    for eptest in range(eval_episode):
        ...
        while not done:
            ...
            store_testing_before_action.append((action, s))
            action = Projection(action, s)
            store_testing_after_action.append((action, s))
...
class DDPG(object):
    ...
    def fw_update(self):
        ...
        for i in range(BATCH_SIZE):
            if abs(bs[i][11:]*ba[i]).sum() <= 20:
                pass
            else:
                ba[i] = Projection(ba[i], bs[i])

        #######let ba in constraint######
        grad = self.get_gradient(bs, ba)
        grad = np.squeeze(grad)
        action_table = np.zeros((BATCH_SIZE, self.a_dim))
        for i in range(BATCH_SIZE):
            with gp.Env(empty=True) as env:
                ...
                with gp.Model(env=env) as fw_m:
                    ...
                    a2 = fw_m.addVar(lb=-1, ub=1, name="a2",
                                     vtype=GRB.CONTINUOUS)
                    a3 = fw_m.addVar(lb=-1, ub=1, name="a3",
                                     vtype=GRB.CONTINUOUS)
                    a4 = fw_m.addVar(lb=-1, ub=1, name="a4",
                                     vtype=GRB.CONTINUOUS)
                    a5 = fw_m.addVar(lb=-1, ub=1, name="a5",
                                     vtype=GRB.CONTINUOUS)
                    a6 = fw_m.addVar(lb=-1, ub=1, name="a6",
                                     vtype=GRB.CONTINUOUS)
                    w1 = bs[i][11]
                    w2 = bs[i][12]
                    w3 = bs[i][13]
                    ...
                    w5 = bs[i][15]
                    w6 = bs[i][16]
                    v = fw_m.addVar(ub=20, name="v")
                    u1 = fw_m.addVar(lb=-gp.GRB.INFINITY, name="u1")
                    u2 = fw_m.addVar(lb=-gp.GRB.INFINITY, name="u2")
                    u3 = fw_m.addVar(lb=-gp.GRB.INFINITY, name="u3")
                    u4 = fw_m.addVar(lb=-gp.GRB.INFINITY, name="u4")
                    u5 = fw_m.addVar(lb=-gp.GRB.INFINITY, name="u5")
                    u6 = fw_m.addVar(lb=-gp.GRB.INFINITY, name="u6")
                    abs_u1 = fw_m.addVar(ub=20, name="abs_u1")
                    abs_u2 = fw_m.addVar(ub=20, name="abs_u2")
                    abs_u3 = fw_m.addVar(ub=20, name="abs_u3")
                    abs_u4 = fw_m.addVar(ub=20, name="abs_u4")
                    abs_u5 = fw_m.addVar(ub=20, name="abs_u5")
                    abs_u6 = fw_m.addVar(ub=20, name="abs_u6")
                    obj = a1*grad[i][0]+a2*grad[i][1]+a3*grad[i][2] + \
                        a4*grad[i][3]+a5*grad[i][4]+a6*grad[i][5]
                    fw_m.setObjective(obj, GRB.MAXIMIZE)

                    fw_m.addConstr(u1 == a1*w1)
                    fw_m.addConstr(u2 == a2*w2)
                    fw_m.addConstr(u3 == a3*w3)
                    fw_m.addConstr(u4 == a4*w4)
                    fw_m.addConstr(u5 == a5*w5)
                    fw_m.addConstr(u6 == a6*w6)
                    fw_m.addConstr(abs_u1 == (gp.abs_(u1)))
                    fw_m.addConstr(abs_u2 == (gp.abs_(u2)))
                    fw_m.addConstr(abs_u3 == (gp.abs_(u3)))
                    fw_m.addConstr(abs_u4 == (gp.abs_(u4)))
                    fw_m.addConstr(abs_u5 == (gp.abs_(u5)))
                    fw_m.addConstr(abs_u6 == (gp.abs_(u6)))
                    fw_m.addConstr((abs_u1 + abs_u2 + abs_u3 +
...
for ep in range(1000000):
    ...
    while not done:
        ...
        if ddpg.pointer < 10000:
            ...
            store_before_action.append((action, s))
        else:
            action = ddpg.choose_action(s)
            store_before_action_and_gaussain.append((action, s))
            action = action + \
                np.random.normal(0, 0.1, a_dim).clip(-max_action, max_action)
            store_before_action.append((action, s))
        action = Projection(action, s)
        store_after_action.append((action, s))
...
np.save("Halfcheetah_{}_NFWPO_Reward_State_relate_cons".format(arg_seed), reward)
np.save("Halfcheetah_{}_NFWPO_before_Action_State_relate_cons".format(
    arg_seed), store_before_action)
np.save("Halfcheetah_{}_NFWPO_after_Action_State_relate_cons".format(
    arg_seed), store_after_action)
np.save("Halfcheetah_{}_NFWPO_eval_reward_State_relate_cons".format(
    arg_seed), eva_reward)
np.save("Halfcheetah_{}_NFWPO_before_Action_Gaussian_State_relate_cons".format(
    arg_seed), store_before_action_and_gaussain)
np.save("Halfcheetah_{}_NFWPO_eval_before_Action_State_relate_cons".format(
    arg_seed), store_testing_before_action)
np.save("Halfcheetah_{}_NFWPO_eval_After_Action_State_relate_cons".format(
