...
class FOCOPS:
    ...
    def update_params(self, rollout, dtype, device):
        ...
        np.save('focops_results_half/avg_cost_state_{}'.format(random_seed), avg_costs)
        # Update nu
...
def train(args):
    ...
    for iter in range(args.max_iter_num):
        ...
        np.save('focops_results_half/network_states_state_{}'.format(random_seed), network_states_)
        network_actions_.append(rollout['network_actions'])
        np.save('focops_results_half/network_actions_state_{}'.format(random_seed), network_actions_)
        ...
        if( (iter+1) % 5 ==0):	
            eval_rewards.append(data_generator.evaluation(env, agent.policy, agent.value_net, agent.cvalue_net,
                                          ...
                                          args.gamma, args.c_gamma, args.gae_lam, args.c_gae_lam,
                                          dtype, device, args.constraint))
        np.save('focops_results_half/eval_reward_state_{}'.format(random_seed), eval_rewards)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env-id', default='HalfCheetah-v3',
    ...
    parser.add_argument('--nu_lr', type=float, default=0.001,
